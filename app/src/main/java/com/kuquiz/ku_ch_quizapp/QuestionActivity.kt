...
class QuestionActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.hasExtra("type_num")){
            ...
            Log.d("review", "오답 노트 확인을 시작합니다.")
            review_check = intent.getIntExtra("review_check", 0)
            if (review_check == 1) {
                question_submit.text = "돌아가기"
                submited = true
                var questions = db_helper.get_review()
                questions.sortByDescending {it.cnt}
                adapter.listData = questions
                adapter.is_review = true
